...
class ParametersTypeSplitter {
  ...
  List<Map<String, dynamic>> _positionalParameters() {
    ...
    return splitParameters
        .map((parameter) => {"type": ParameterType.positional, "declaration": parameter.withoutWhiteSpaces() })
        .toList();
  }
  ...
  List<Map<String, dynamic>> _optionalPositionalParameters() =>
      _getParametersEnclosedIn(Delimiters.SQUARED_BRACKETS)
          .map((parameter) => {"type": ParameterType.optionalPositional, "declaration": parameter.withoutWhiteSpaces()})
          .toList();

  List<Map<String, dynamic>> _namedParameters() =>
      _getParametersEnclosedIn(Delimiters.CURLY_BRACKETS)
          .map((parameter) => {"type": ParameterType.named, "declaration": parameter.withoutWhiteSpaces()})
          .toList();
